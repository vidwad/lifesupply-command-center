...
export type SalesOverview = {
  ...
  orderCount: number;
  /**
   * Billed revenue from order headers — `grandTotal`, so inclusive of tax and
   * shipping. Independent of line-item coverage, and therefore the only
   * revenue figure here that is trustworthy today.
   */
  revenue: number;
  /**
...
export async function getSalesOverview(
  ...
  const [orderAgg, ordersWithItems, lineAgg, costedAgg] = await Promise.all([
  ...
  const revenue = numOrZero(orderAgg._sum.grandTotal);
  ...
  return {
    ...
    revenue,
    productRevenue,
    averageOrderValue: orderCount > 0 ? Math.round((revenue / orderCount) * 100) / 100 : null,
