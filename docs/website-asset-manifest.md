...
| People aged 65 and older, Canada and the United States | `demographic-chart.tsx` | Medical Supplies, "Aging populations. Ongoing supply needs." | Two labelled lists of year and value from `content/businesses.ts`; the projection row is a dashed outline carrying no figure; legend, sources and qualification are text | Typeset in tokens, no image file. Added 2026-09-13. Every figure is recorded in `docs/website-content-evidence.md`, "Market context"; a canary holds the block to those figures and to no market-growth or revenue claim |
