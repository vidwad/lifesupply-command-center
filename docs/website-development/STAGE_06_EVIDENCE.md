...
- **Post-merge verification owed:** confirm `GET https://lifesupply-cc-web.onrender.com/api/public/v1/news` returns 200 with an empty list, that the production alias `/news` shows the empty note after revalidation, and exercise create → submit → approve → publish → withdraw once with two users on Render; record it in STATUS.
