...
| ID | Requirement | Status | Accountable role | Evidence required | Evidence location | Blocker / dependency | Target date | Decision / approval | Notes |
|---|---|---|---|---|---|---|---|---|---|
| 11B-01 | Provision staging web, worker, cron, database, Inngest, storage | Not Started | Developer / Technical Admin | Staging resource inventory with service IDs | TBD | `DEC-01`, `DEC-02`, `DEC-03` | TBD (Week 1) | TBD | Staging does not exist today |
| 11B-02 | Environment-safe blueprint separation or controlled duplicate process | Not Started | Developer / Technical Admin | Updated `render.yaml` or documented duplication procedure | TBD | 11B-01 | TBD (Week 1) | TBD | Current `render.yaml` defines production-named resources only |
| 11B-03 | Confirm `DATABASE_URL` / `DIRECT_URL` behaviour for web, worker, migration, cron | Not Started | Developer / Technical Admin | Per-process connection matrix | TBD | 11B-01 | TBD (Week 1) | TBD | See blocker `BLK-03` — CI sets no `DIRECT_URL` |
| 11B-04 | Set and validate all staging environment variables | Not Started | Developer / Technical Admin | Staging variable checklist, values not recorded | TBD | 11B-01, `DEC-06`, `DEC-07` | TBD (Week 1) | TBD | Secrets must never be written into this repository |
| 11B-05 | Run every committed migration on staging | Not Started | Developer / Technical Admin | Migration log showing 21/21 applied on a clean staging DB | TBD | 11B-01 | TBD (Week 2) | TBD | 21 migrations exist at baseline |
| 11B-06 | Seed only approved staging users, roles, flags, synthetic data | Not Started | Developer / Technical Admin | Seed run log and account inventory | TBD | 11B-01 | TBD (Week 2) | TBD | No real customer data in staging |
| 11B-07 | Post-deployment staging smoke-test workflow | Not Started | Developer / Technical Admin | Smoke-test workflow file and a passing run | TBD | 11B-01 | TBD (Week 2) | TBD | Does not exist at baseline |
| 11B-08 | Release approval and rollback steps in the pipeline | Not Started | Technical Lead | Documented approval gate and rehearsed rollback | TBD | 11B-01 | TBD (Week 2) | TBD | Complements `docs/OPS_RUNBOOK.md` §7 |
| 11B-09 | Prevent automatic production deploy from every merge to `main` | Not Started | Technical Lead | Render auto-deploy configuration evidence | TBD | `DEC-12` | TBD (Week 2) | TBD | `render.yaml` currently sets `autoDeploy: true` on `main` — see `BLK-01` |
| 11B-10 | Staging deployment recreatable from repository instructions | Not Started | Developer / Technical Admin | Clean-room rebuild performed from docs alone | TBD | 11B-01…11B-08 | TBD (Week 2) | TBD | Acceptance criterion |
| 11B-11 | Web, DB, worker, cron, Inngest all healthy in staging | Not Started | Developer / Technical Admin | Health screenshots/logs per service | TBD | 11B-01 | TBD (Week 2) | TBD | Acceptance criterion |
| 11B-12 | `/api/health` exposes only non-sensitive information | Not Started | Developer / Technical Admin | Reviewed health-endpoint response body | TBD | 11B-01 | TBD (Week 2) | TBD | Cross-check `docs/16_DEPLOYMENT_AND_ENVIRONMENT.md` §12 |
| 11B-13 | A background job reaches a terminal status in staging | Not Started | Developer / Technical Admin | Sync log transitioning `running` → terminal state | TBD | 11B-01 | TBD (Week 2) | TBD | Guards against the stalled-job failure mode |
| 11B-14 | Migration and rollback procedures documented and rehearsed | Not Started | Technical Lead | Rehearsal record with date and outcome | TBD | 11B-05 | TBD (Week 2) | TBD | Feeds `docs/OPS_RUNBOOK.md` §5, §7 |
...
|---|---|---|---|---|---|---|
| BLK-01 | `render.yaml` sets `autoDeploy: true` on `main` for web, worker, and cron, so every merge deploys the live environment. This conflicts with Phase 11B requirement "prevent automatic production deployment directly from every merge to `main`" and with the controlled-cutover model in 11G. | Unreviewed code can reach the live environment without a release gate. | 2026-08-04 | Technical Lead | Resolve in 11B: disable auto-deploy on production services, or repoint them to a release branch. Requires `DEC-12`. | Open |
| BLK-02 | No staging environment exists. The only deployed environment is the live Render stack, so every Phase 11B–11F evidence item currently has nowhere to run. | Blocks the majority of Phase 11 evidence. | 2026-08-04 | Product Owner | Provision staging in 11B. Requires `DEC-01`, `DEC-02`, `DEC-03`. | Open |
| BLK-03 | `.github/workflows/ci.yml` sets no `DIRECT_URL`, but `prisma/schema.prisma` declares `directUrl = env("DIRECT_URL")`. CI passes today only because `prisma generate` does not resolve `directUrl`; any CI step invoking `prisma migrate`/`validate` would fail with P1012. | Latent CI break the first time migration verification is added to the pipeline. | 2026-08-04 | Developer / Technical Admin | Add a schema-only `DIRECT_URL` to the CI env block in 11B. | Open |
...
| BLK-07 | Object storage is referenced throughout the plan and `docs/16_DEPLOYMENT_AND_ENVIRONMENT.md` §6, but no `S3_*` values are configured in `render.yaml` for any environment. | Supplier evidence, exports, and report artefacts may not persist as designed. | 2026-08-04 | Technical Lead | Confirm storage requirement and provision in 11B. Requires `DEC-03`. | Open |
