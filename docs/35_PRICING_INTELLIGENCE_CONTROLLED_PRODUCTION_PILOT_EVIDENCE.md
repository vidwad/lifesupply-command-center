...
> | Products never synced — 14 seed products, BB V6222 absent (`F-4`) | **Outstanding** |
...
4. **`F-4`** — run a BigCommerce product sync so the test SKU exists as a BigCommerce-mapped variant. The upload path supplies cost but **cannot** substitute for the sync: the writeback resolves its target from `Product.sourceId` / `ProductVariant.sourceId` on the database rows, not from the CSV's id columns. Note this is a first-time import of ~36,000 products into production. — *Developer*
