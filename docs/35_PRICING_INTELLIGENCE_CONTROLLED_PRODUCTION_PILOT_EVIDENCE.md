...
> | Products never synced (`F-4`) | **Cleared 2026-08-23** — full sync imported **51,114** products / 50,053 variants across both stores. A *full* sync bypasses the `F-10` 422, which only affects incremental |
...
| **F-16** | *(financial display)* | **A cost of zero was reported as a real cost of $0.00, producing a 100% margin.** `listProducts` tested `primary?.costPrice ? … : …`, but a Prisma `Decimal` is an object and `Decimal(0)` is truthy, so an unpopulated cost became a known cost of zero. Confirmed in production on BB V6222: price $170.44, cost **$0.00**, margin **100.0%** | **50,024 of 50,053** production variants have `costPrice = 0`, so effectively the entire catalogue reported a fabricated 100% margin on a management screen. Margin is a financial figure and this one was invented by a truthiness bug. **Fixed**: a cost now counts only if strictly positive, falling through to the preferred supplier cost and then to null, matching `positive()` in the pricing engine and the upload parser's "treats a zero cost as absent, never as a zero floor" | **Defect — fixed, not yet merged** | Developer |
| **F-13** | `F-4` prerequisite | **The catalog sync never requested product images, so every product would import flagged `missing` image.** `deriveImageStatus` reads `bc.images`, but the v3 catalog list endpoint omits the key entirely unless `include=images` is passed — and the sync only ever passed `include=variants`. Verified against the live API: with the old query `images` is `undefined`; with `include=variants,images` BB V6222 returns 1 image. **249 of 250 sampled products do have an image** | Would have flagged all ~36,000 products as missing an image, making the *Missing images* and *Needs review* catalogue-quality filters useless and the counts meaningless. Reported by the product owner, who found BB V6222 marked as having no image when it has one. **Fixed** on `claude/products-pagination-and-image-status` | **Defect — fixed, not yet merged** | Developer |
| **F-14** | *(list UI)* | **`/products` capped the list at a hardcoded `take: 100` with no pagination, and rendered `products.length` as the total.** It also ignored the shell's **Division** selector entirely — `?division=` had no effect on the list | With 14 seed rows this was invisible. Against a 36,003-product catalogue the header would have read **"100 products"** with no way to reach the other 35,903 — silent truncation presented as a complete result, the same failure shape as `F-11`. **Fixed** on the same branch: server-side paging at 50/page matching Customers and Orders, a real `countProducts()` total, division scoping, and filters preserved across pills, search and paging | **Defect — fixed, not yet merged** | Developer |
| **F-15** | *(scope note)* | The **Division** selector in the app shell is consumed by the Financials pages and financial exports only. Customers, Orders and the Dashboard still ignore it | The selector reads as a global filter but applies to a minority of pages, so a user can believe a page is scoped when it is not. Products and the sync buttons are fixed on the two open branches; **Customers, Orders and Dashboard remain unwired** | Open — follow-up | Product Owner to prioritise |
